...
R_U <- function(A, U, x, gamma_G, sigma_A) {
  #,a_D,gamma_D){
  gamma_G * U[x] * A[x] / (1 + sigma_A * A[x] ^ 2)
}
####################################################################
...
calculateSpreadRate <-
  function(u_0,
           ...
           L=0) {
    A0 <- rep(0,n)
    A0[round(n / 2-L*n/(xmax-xmin)):round(n / 2+L*n/(xmax-xmin))] <- beta
    U0 <- rep(u_0, n)
...
critical_U <- function(maxU,Tmax,delta_A,
                       gamma_A,
                       sigma_A,
                       gamma_S,
                       beta,
  ...
  while(i<imax && abs(spreadRate)>tol){
    spreadRate <- calculateSpreadRate((a+b)/2,Tmax,
                                      delta_A,
                                      gamma_A,
                                      sigma_A,
                                      gamma_S,
