...
priors <- list(c('unif',0,1),
  c('unif',0,1),#c('unif',0,100),
  c('unif',0,100),#c('unif',0,100),
               c('unif',0,100),
               c('unif',0,10),c('unif',0,1),
               c('unif',0,10),c('unif',0,100))#,
...
R_U <- function(A,U,x,gamma_G,sigma_A){#,a_D,gamma_D){
  G <- gamma_G*U[x]*A[x]/(1+sigma_A*A[x]^2)
...
IDE_procedure <- function(A_0,U_0,Tmax=12,delta_A,gamma_A,sigma_A,gamma_S,beta,a_A,R,delta_U,gamma_G,a_U){
  A <- rbind(NULL, A_0)
  U <- rbind(NULL, U_0)
  n <- length(region)
  ...
  for(t in 1:Tmax){
    ...
    A_hat <- sapply(X=1:n,FUN=P_A,A=A[t,],U=U[t,],delta_A=delta_A,gamma_A=gamma_A,sigma_A=sigma_A)+
      sapply(X=1:n,FUN=S,A=A[t,],U=U[t,],gamma_S=gamma_A,beta=beta)*
      sapply(X=(1:n)*(xmax-xmin)/n+xmin,FUN = wSimpson,w = spores_A,f = k,xmin = xmin,xmax = xmax,a=a_A)
    A <- rbind(A,A_hat)
    
    spores_U <- sapply(X=1:n,FUN=R_U,A=A[t,],U=U[t,],gamma_G=gamma_G,sigma_A=sigma_A)#,gamma_D=params[12],a_D=params[13])
...
summaryStatistics <- function(A,A_0){
 c(norm(A-A_0,"2")/norm(A_0,'2'),abs(sum(A)-sum(A_0)))
}
...
model <- function(params){
  ...
  library(dplyr)
  set.seed(params[1])
  source(paste(here(),'/Code/sigma0/ABC/globalVariables.R',sep=''))
  source(paste(here(),'/Code/sigma0/ABC/model.R',sep=''))
  ...
  # if(obtainedParameters==1){
    ...
    sigma_A <- 0
    gamma_S <- params[4]
    beta <- estimatedbeta[currentRegion]
    a_A <- params[5]
    R <- params[6]
    delta_U <- params[7]
    gamma_G <- params[8]
  ...
  # browser()
    ...
    IDE_procedure(
      ...
      prior_U,
      Tmax=12,
      delta_A,gamma_A,sigma_A,gamma_S,beta,a_A,R,delta_U,gamma_G,a_U
    )
